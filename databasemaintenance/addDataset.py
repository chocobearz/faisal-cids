...
for i, table in enumerate(timePoint):
  for col in table:
    ...
    if column in previousTable[i]:
      ...
      print(f'''One of the columns: {col} you are attempting to load is already in 
      ...
      and if the new table is correct manually move the column to the new table {tables[i]}''')
      adjTimePoint[i].remove(col)
      timePoint[i].remove(col)
